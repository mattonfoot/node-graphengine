...
const render = (g) => {
  ...
  var relUri = path.relative(path.dirname(id.pathname + '.html'), path.dirname(id.pathname + '.html'));
  var uri = path.join(relUri, path.basename(id.pathname + '.html'));

  html.push('<html xmlns="http://www.w3.org/1999/xhtml">');
  html.push('<head>');
  html.push('<title>' + model.label + '</title>');
  html.push('<link rel="alternate" type="application/rdf+json" href="' + path.basename(id.pathname + '.json') + '" title="RSS feed for this page"/>');
  html.push('<link rel="alternate" type="application/turtle" href="' + path.basename(id.pathname + '.ttl') + '" title="RSS feed for this page"/>');
  html.push('</head>');
  html.push('<body>');
  html.push('<div typeof="' + model.type + '" about="' + model.id + '" style="width:960px;margin:0 auto;">');
  ...
  html.push('<table cellspacing="0" cellpadding="5" border="1" width="100%">');
  ...
  for (var p in model.owl) {
    ...
    html = html.concat([
      '\t<tr>',
        '\t\t<th valign="top" align="right" width="20%">' + (p === currentPredictate ? '&nbsp;' : p) + '</th>',
